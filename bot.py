...
class UserDatabase:
    ...
    def can_use_voice(self, user_id):
        ...
        else: return user['voice_uses_today'] < 3
    ...
    def get_voice_uses_left(self, user_id):
        user = self.get_user(user_id)
        if not user or user['is_premium']: return "∞"
...
class AIChatBot:
    ...
    def initialize_gemini_models(self):
        try:
            ...
            for model in priority_models:
                if model in chat_models: models_to_try.append(model)
            if not models_to_try and chat_models: models_to_try = chat_models[:5]
            if not models_to_try: models_to_try = priority_models
            safety_settings_standard = [{"category": "HARM_CATEGORY_HARASSMENT", "threshold": "BLOCK_MEDIUM_AND_ABOVE"}, {"category": "HARM_CATEGORY_HATE_SPEECH", "threshold": "BLOCK_MEDIUM_AND_ABOVE"}, {"category": "HARM_CATEGORY_SEXUALLY_EXPLICIT", "threshold": "BLOCK_MEDIUM_AND_ABOVE"}, {"category": "HARM_CATEGORY_DANGEROUS_CONTENT", "threshold": "BLOCK_MEDIUM_AND_ABOVE"}]
            safety_settings_premium = [{"category": "HARM_CATEGORY_HARASSMENT", "threshold": "BLOCK_ONLY_HIGH"}, {"category": "HARM_CATEGORY_HATE_SPEECH", "threshold": "BLOCK_MEDIUM_AND_ABOVE"}, {"category": "HARM_CATEGORY_SEXUALLY_EXPLICIT", "threshold": "BLOCK_ONLY_HIGH"}, {"category": "HARM_CATEGORY_DANGEROUS_CONTENT", "threshold": "BLOCK_MEDIUM_AND_ABOVE"}]
            ...
            if self.gemini_model_standard or self.gemini_model_premium:
                model_names = []
                if self.gemini_model_standard: model_names.append("стандартная")
                if self.gemini_model_premium: model_names.append("премиум")
                self.model_name = f"Gemini: {', '.join(model_names)}"
            else: logger.error("Все модели Gemini недоступны")
        except Exception as e: logger.error(f"Ошибка инициализации Gemini: {str(e)}")
    ...
    def _initialize_model_with_settings(self, models_to_try, safety_settings, model_type):
        for model_name in models_to_try:
            try:
                ...
                error_str = str(e)
                if "quota" in error_str.lower() or "429" in error_str: break
                continue
        return None
    ...
    def try_gemini_response(self, user_id: int, user_text: str, lang: str, voice_requested: bool = False, is_premium: bool = False, explicit_mode: bool = False, gemini_model=None) -> str:
        ...
        try:
            ...
            for msg in history[-max_history_length:]:
                if msg["role"] == "user": conversation_history += f"Пользователь: {msg['content']}\n"
                else: conversation_history += f"Ты: {msg['content']}\n"
            ...
            if response and response.text:
                ...
                if len(history) > max_history_length: user_conversations[user_id] = history[-max_history_length:]
                return bot_response
        except Exception as e:
    ...
    def clean_response(self, response: str, voice_requested: bool = False) -> str:
        if not response: return "Интересно! Расскажи больше." if not voice_requested else "Интересно, расскажи больше."
        response = response.replace('*', '').replace('**', '').strip()
        if response.startswith('Ты:') or response.startswith('You:'): response = response.split(':', 1)[1].strip()
        if voice_requested: response = self.remove_emojis(response)
        if response and not response.endswith(('.', '!', '?')): response += '.'
        if len(response) < 2: return "Расскажи мне больше об этом!" if not voice_requested else "Расскажи мне больше об этом."
        return response
    ...
    def get_enhanced_fallback_response(self, user_id: int, user_text: str, lang: str, voice_requested: bool = False) -> str:
        ...
        max_history_length = 30 if is_premium else 12
        if lang == 'ru':
            if any(word in user_text_lower for word in ['привет', 'здравств', 'добрый', 'hi', 'hello', 'хай', 'ку']): responses = ["Привет! Рада тебя видеть! 😊"]
            elif any(word in user_text_lower for word in ['как дела', 'как ты', 'настроен']): responses = ["Всё прекрасно! А у тебя как? 😉"]
            else: responses = ["Расскажи мне больше об этом! 💫"]
        else: responses = ["Tell me more about it! 💫"]
    ...
    def preprocess_text_for_speech(self, text: str) -> str:
        emoji_replacements = {'))': ', улыбаясь,', ')))': ', смеясь,', ':)': ', улыбаясь,', ':(': ', с грустью,', ';)': ', подмигивая,', '<3': ', с любовью,'}
        for emoji, replacement in emoji_replacements.items(): text = text.replace(emoji, replacement)
    ...
    def text_to_speech(self, text: str, user_id: int, lang: str = 'ru') -> str:
        try:
            ...
            if len(processed_text) > 1000: processed_text = processed_text[:1000] + "..."
            audio_filename = f"voice_{user_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.mp3"
            tts_services = [self.try_silero_tts_improved, self.try_gtts_enhanced]
    ...
    def try_silero_tts_improved(self, text: str, filename: str, lang: str) -> str:
        try:
            if not self.silero_available: return None
            import torch
            ...
            if lang == 'ru':
                ...
                model, _ = torch.hub.load(repo_or_dir='snakers4/silero-models', model='silero_tts', language='en', speaker='v3_en')
                speaker = 'en_0'
                sample_rate = 48000
            model.to(device)
    ...
    def try_gtts_enhanced(self, text: str, filename: str, lang: str) -> str:
        try:
            if lang == 'ru': tts = gTTS(text=text, lang='ru', slow=False, lang_check=False)
            else: tts = gTTS(text=text, lang='en', slow=False, lang_check=False)
            tts.save(filename)
...
def create_crypto_invoice(amount: float, currency: str = "USDT") -> dict:
    try:
        ...
        payload = {"asset": currency, "amount": str(amount), "description": "Premium subscription for 1 week", "hidden_message": "Thank you for your purchase!", "paid_btn_name": "callback", "paid_btn_url": "https://t.me/your_bot_username", "payload": "premium_subscription", "allow_comments": False, "allow_anonymous": False}
...
def check_crypto_payment(invoice_id: int) -> bool:
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            if data.get("ok") and data.get("result", {}).get("items"): return len(data["result"]["items"]) > 0
        return False
...
def start_command(message):
    ...
    if not user_db.get_user(user_id): user_db.create_user(user_id, message.from_user.username, message.from_user.first_name, message.from_user.last_name)
...
def premium_command(message):
    ...
    user_data = user_db.get_user(user_id)
    if not user_data: user_db.create_user(user_id, message.from_user.username, message.from_user.first_name, message.from_user.last_name); user_data = user_db.get_user(user_id)
    ...
    if lang == 'ru':
        if is_premium:
            ...
            keyboard.add(types.InlineKeyboardButton("💫 50 Stars", callback_data='buy_premium_stars'), types.InlineKeyboardButton("💰 CryptoBot", callback_data='buy_premium_crypto'))
    else:
        if is_premium:
            premium_until = user_data['premium_until']
            ...
            keyboard = types.InlineKeyboardMarkup(row_width=2)
            keyboard.add(types.InlineKeyboardButton("💫 50 Stars", callback_data='buy_premium_stars'), types.InlineKeyboardButton("💰 CryptoBot", callback_data='buy_premium_crypto'))
    if is_premium: bot.send_message(message.chat.id, text, parse_mode='Markdown')
    else: bot.send_message(message.chat.id, text, parse_mode='Markdown', reply_markup=keyboard)
...
def explicit_command(message):
    ...
    if not user_data['is_premium']:
        if lang == 'ru': bot.send_message(message.chat.id, "❌ Эта функция доступна только для премиум пользователей!")
        else: bot.send_message(message.chat.id, "❌ This feature is available only for premium users!")
        return
    new_mode = user_db.toggle_explicit_mode(user_id)
    if lang == 'ru':
        status = "ВКЛЮЧЕН" if new_mode else "ВЫКЛЮЧЕН"
        text = f"🔞 Режим откровенных тем: *{status}*\n\n"
        if new_mode: text += "Теперь я готова к более откровенным беседам 💫\n*Только для 18+*"
        else: text += "Теперь наши беседы будут более сдержанными 💖"
    else:
        status = "ENABLED" if new_mode else "DISABLED"
        text = f"🔞 Explicit mode: *{status}*\n\n"
        if new_mode: text += "Now I'm ready for more open conversations 💫\n*For 18+ only*"
        else: text += "Now our conversations will be more restrained 💖"
...
def profile_command(message):
    ...
    if lang == 'ru':
        ...
        if user_data['is_premium']: text += f"• Откровенный режим: {'ВКЛЮЧЕН' if user_data['explicit_mode'] else 'ВЫКЛЮЧЕН'}\n"
        ...
        if user_data['is_premium']: text += f"• Explicit mode: {'ENABLED' if user_data['explicit_mode'] else 'DISABLED'}\n"
...
def voice_command(message):
    ...
    if lang == 'ru': text = f"🔉 Голосовые сообщения: *{'ВКЛЮЧЕНЫ' if new_status else 'ВЫКЛЮЧЕНЫ'}*"
    else: text = f"🔉 Voice messages: *{'ENABLED' if new_status else 'DISABLED'}*"
    bot.send_message(message.chat.id, text, parse_mode='Markdown')
...
def buy_premium_stars_callback(call):
    ...
    try:
        ...
        bot.send_invoice(chat_id=call.message.chat.id, title="Премиум подписка на 1 неделю" if lang == 'ru' else "Premium Subscription (1 week)", description="Активация премиум подписки на 1 неделю." if lang == 'ru' else "Premium subscription for 1 week.", invoice_payload=f"premium_{user_id}", provider_token="", currency="XTR", prices=prices, start_parameter="premium_subscription", need_name=False, need_phone_number=False, need_email=False, need_shipping_address=False, is_flexible=False)
        if lang == 'ru': bot.answer_callback_query(call.id, "💳 Открываю окно оплаты...")
        else: bot.answer_callback_query(call.id, "💳 Opening payment window...")
    except Exception as e:
        if lang == 'ru': bot.answer_callback_query(call.id, "❌ Ошибка при создании платежа")
        else: bot.answer_callback_query(call.id, "❌ Error creating payment")
...
def buy_premium_crypto_callback(call):
    ...
    if CRYPTO_BOT_TOKEN == "ВАШ_CRYPTOBOT_API_ТОКЕН":
        if lang == 'ru': bot.answer_callback_query(call.id, "❌ Оплата через CryptoBot временно недоступна")
        else: bot.answer_callback_query(call.id, "❌ CryptoBot payment temporarily unavailable")
        return
    try:
        ...
        if invoice and invoice.get('pay_url'):
            ...
            if lang == 'ru': bot.answer_callback_query(call.id, "💳 Создаем платеж...")
            else: bot.answer_callback_query(call.id, "💳 Creating payment...")
        else:
            if lang == 'ru': bot.answer_callback_query(call.id, "❌ Ошибка при создании платежа")
            else: bot.answer_callback_query(call.id, "❌ Error creating payment")
    except Exception as e:
...
def pre_checkout_handler(pre_checkout_query):
    ...
    try:
        if payload.startswith('premium_'): bot.answer_pre_checkout_query(pre_checkout_query.id, ok=True)
        else: bot.answer_pre_checkout_query(pre_checkout_query.id, ok=False, error_message="Неизвестный тип платежа")
    except Exception as e: bot.answer_pre_checkout_query(pre_checkout_query.id, ok=False, error_message="Ошибка обработки платежа")
...
def successful_payment_handler(message):
    ...
    try:
        ...
        if lang == 'ru': bot.send_message(message.chat.id, "❌ Произошла ошибка при активации премиума.")
        else: bot.send_message(message.chat.id, "❌ Error activating premium.")

def should_send_voice_message(user_text: str, lang: str) -> tuple:
    text_lower = user_text.lower().strip()
    if lang == 'ru':
        patterns = [r'.*скинь\s+войс\s*[.!?]*$', r'.*отправь\s+войс\s*[.!?]*$', r'.*ответь\s+голосом\s*[.!?]*$', r'.*войс\s*[.!?]*$', r'.*озвучь\s*[.!?]*$']
        ...
        patterns = [r'.*send\s+voice\s*[.!?]*$', r'.*send\s+voice\s+message\s*[.!?]*$', r'.*respond\s+with\s+voice\s*[.!?]*$', r'.*voice\s+message\s*[.!?]*$', r'.*voice\s*[.!?]*$']
        for pattern in patterns:
            if re.match(pattern, text_lower):
...
def send_voice_message(chat_id: int, audio_file: str, user_id: int) -> bool:
    try:
        with open(audio_file, 'rb') as voice_file: bot.send_audio(chat_id, voice_file, title="Голосовое сообщение")
        chat_voice_support[chat_id] = True
        return True
    except Exception as e:
        error_msg = str(e)
        if "Voice_messages_forbidden" in error_msg or "voice messages are forbidden" in error_msg.lower(): chat_voice_support[chat_id] = False
        else: user_voice_enabled[user_id] = False
        return False
...
def handle_message(message):
    ...
    if not cleaned_text.strip(): cleaned_text = "Привет" if lang == 'ru' else "Hello"
            ...
            try:
                ...
                if voice_success:
                    ...
                    if not user_data['is_premium']:
                        ...
                        if uses_left > 0:
                            if lang == 'ru': reminder = f"🔔 Осталось войсов сегодня: {uses_left}/3\n💫 Безлимитные войсы с /premium"
                            else: reminder = f"🔔 Voice messages left today: {uses_left}/3\n💫 Unlimited voice with /premium"
                            bot.send_message(chat_id, reminder)
                if not voice_success:
                    if chat_voice_support[chat_id]:
                        if lang == 'ru': bot.send_message(chat_id, "⚠️ Не удалось отправить голосовое сообщение.")
                        else: bot.send_message(chat_id, "⚠️ Couldn't send voice message.")
                    else:
                        if lang == 'ru': bot.send_message(chat_id, "🔔 В этом чате голосовые сообщения запрещены.")
                        else: bot.send_message(chat_id, "🔔 Voice messages are forbidden in this chat.")
            except Exception as e: bot.send_message(chat_id, bot_response)
            finally:
                if os.path.exists(audio_file):
                    try: os.remove(audio_file)
                    except Exception as e: pass
        else: bot.send_message(chat_id, bot_response)
    else:
        bot.send_message(chat_id, bot_response)
        if send_voice and not user_db.can_use_voice(user_id):
            user_data = user_db.get_user(user_id)
            if not user_data['is_premium']:
                if lang == 'ru': bot.send_message(chat_id, f"❌ Лимит войсов исчерпан! 3/3 войсов сегодня.\n\n💫 Премиум пользователи имеют безлимитные войсы!\n/premium - активация за 50 Stars")
                else: bot.send_message(chat_id, f"❌ Voice message limit reached! 3/3 today.\n\n💫 Premium users get unlimited voice!\n/premium - activate for 50 Stars")
...
if __name__ == '__main__':
    ...
    except Exception as e: print(f"Ошибка при удалении вебхука: {e}")
