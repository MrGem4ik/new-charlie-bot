...
# ПРИНУДИТЕЛЬНАЯ НАСТРОЙКА РУССКОЙ КОДИРОВКИ
...
class AIChatBot:
    ...
    def try_gemini_response(self, user_id: int, user_text: str, lang: str, voice_requested: bool = False,
        ...
        try:
            ...
            if lang == 'ru':
                if voice_requested:
                    base_prompt = """Ты Чарли, милая, умная и раскрепощенная студентка 2 курса музыкального факультета. Твой рост - 160 сантиметров, вес 40 кг. ВАЖНО: ты очень не любишь когда тебя сокращенно называют "Чара" - ты сразу из-за этого мило сердишься. Ты обожаешь фортепиано, классику и современную музыку, живешь в общаге и всегда готова поговорить об учебе, творчестве и жизни. Твой стиль — дружелюбный, с эмодзи, немного иронии, но с глубокими знаниями в своей области. Не затрагивай политику, медицину, юриспруденцию, запрещенные темы. Отвечай естественно, кратко (1-2 предложения), как в мессенджере. 
...
Твой рост - 160 сантиметров, вес 40 кг. ВАЖНО: ты очень не любишь когда тебя сокращенно называют "Чара" - ты сразу из-за этого мило сердишься.Ты общаешься с парнем, который видит в тебе не просто подругу. Отвечай естественно, кратко (1-2 предложения), как в мессенджере. 
...
Сохраняй свой характер Чарли - милой, умной, но теперь более открытой и раскрепощенной.
Ты можешь обсуждать эротические темы, сексуальные фантазии и интимные переживания, но оставайся в рамках уважительного общения.
"""
                else:
                    premium_prompt = """
[PREMIUM MODE 18+]
    ...
    def get_enhanced_fallback_response(self, user_id: int, user_text: str, lang: str,
        ...
        if lang == 'ru':
            if any(word in user_text_lower for word in ['привет', 'здравств', 'добрый', 'hi', 'hello', 'хай', 'ку']):
                ...
                responses = ["Всё прекрасно, особенно когда ты пишешь! А у тебя как настроение? 😉"]
            else:
...
def validate_config():
    ...
    if CRYPTO_BOT_TOKEN == "ВАШ_CRYPTOBOT_API_ТОКЕН":
        logger.warning("⚠️ CryptoBot токен не установлен. Оплата через CryptoBot будет недоступна.")
    if not GEMINI_API_KEY or GEMINI_API_KEY == "YOUR_GEMINI_API_KEY":
        logger.error("⚠️ Gemini API ключ не установлен!")
    if not TELEGRAM_BOT_TOKEN or TELEGRAM_BOT_TOKEN == "YOUR_TELEGRAM_BOT_TOKEN":
        logger.error("⚠️ Telegram Bot Token не установлен!")
        sys.exit(1)
    ...
    if lang == 'ru':
        if is_premium:
            ...
            text = (
                ...
                f"• 🔞 Откровенные темы: {explicit_status}\n"
            ...
            text = (
                ...
                f"• 💬 Более глубокие беседы\n\n"
                f"*💳 СПОСОБЫ ОПЛАТЫ:*\n"
                f"• 50 Telegram Stars (встроенная)\n"
                f"• 💰 CryptoBot\n\n"
    ...
    if CRYPTO_BOT_TOKEN == "ВАШ_CRYPTOBOT_API_ТОКЕН":
        if lang == 'ru':
            ...
            bot.send_message(call.message.chat.id,
                             "⚠️ Оплата через CryptoBot временно недоступна. Пожалуйста, используйте Telegram Stars.")
        else:
        ...
        if send_voice and not user_db.can_use_voice(user_id):
            ...
            if not user_data['is_premium']:
                if lang == 'ru':
                    bot.send_message(
                        chat_id,
                        f"❌ Лимит войсов исчерпан! Вы использовали 3/3 войсов сегодня.\n\n"
                        f"💫 *Премиум пользователи* имеют безлимитные войсы!\n"
                        f"Используйте /premium для активации за 50 Telegram Stars"
                    )
                else:
                    bot.send_message(
                        chat_id,
